...
def verbose_ping(hostname, timeout=WAIT_TIMEOUT, count=NUM_PACKETS,
    ...
    try:
        ...
        print("\nPING: Unknown host: %s (%s)" % (hostname, e.args[1]))
        print()
        return False
...
def main():
    ...
    verbose_ping(args.destination, timeout=args.timeout*1000, count=args.count,
         packet_size=args.packet_size)

def is_up(addres, timeout):
    rez = verbose_ping(addres, timeout)
    return rez

def pinging(addres, queue, timeout):
    try:
...
# def get_broadcasting(addr, mask):
    # return ipaddress.IPv4Network(addr + "/" + mask, False).broadcast_address
